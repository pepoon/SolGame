...
namespace DOTSNET
{
    ...
    public class NetworkServerSystem : SystemBase
    {
        ...
        public NativeMultiHashMap<int, Entity> ownedPerConnection;
        ...
        public NativeHashMap<ulong, Entity> spawned;

        // snapshot state updates in the given interval
        public float snapshotInterval = 0.050f;
        double lastSnapshotTime = 0;
        // SnapshotMessages cached so we can use burst
        NativeList<SnapshotMessageAndConnectionId> snapshotMessages;
                ...
                if (identity.netId != 0)
                {
                    ...
                    if (spawned.Remove(identity.netId))
                    {
                        ...
                        for (int i = 0; i < observers.Length; ++i)
                        {
                            ...
                            {
                                // send it
                                Send(observerConnectionId, new UnspawnMessage(identity.netId));
                            }
                        }
        ...
        public void DestroyOwnedEntities(int connectionId)
        {
            ...
            //          allocate here once.
            NativeMultiHashMapIterator<int>? iterator = default;
        ...
        //
        // push-broadcasting
        // makes sense in ECS for now because Entities.ForEach can be bursted!
        // going through .connections and looking for 'observing' would be
        // slower/cumbersome.
        //
        void Broadcast()
        {
            ...
            NativeList<SnapshotMessageAndConnectionId> messages = snapshotMessages;
                              ...
                              in NetworkComponentsSerialization serialization) =>
            {
                if (observers.Length > 0)
                {
                    ...
                    for (int i = 0; i < observers.Length; ++i)
                    {
                        ...
                        messages.Add(new SnapshotMessageAndConnectionId{
                            connectionId = observers[i].connectionId,
                            message = message
                        });
                    }
                }
            ...
            {
                SnapshotMessageAndConnectionId entry = snapshotMessages[i];
                Send(entry.connectionId, entry.message);
            }
        ...
        protected override void OnStartRunning()
        {
            ...
            spawned = new NativeHashMap<ulong, Entity>(1000, Allocator.Persistent);
            snapshotMessages = new NativeList<SnapshotMessageAndConnectionId>(1000, Allocator.Persistent);
            ownedPerConnection = new NativeMultiHashMap<int, Entity>(1000, Allocator.Persistent);
            ...
            if (transport != null)
            {
                ...
                if (lateGroup != null)
                {
                    // TODO use EnableFixedRateSimple after it was fixed. right now
                    // it only sets deltaTime but updates with same max frequency
                    lateGroup.FixedRateManager = new FixedRateUtils.FixedRateCatchUpManager(tickInterval);
